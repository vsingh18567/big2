...
from simulator.cards import Combo, PASS, card_name
...
def select_action(policy: MLPPolicy, state: np.ndarray, candidates: list[Combo]) -> tuple[Combo, torch.Tensor, torch.Tensor, torch.Tensor]:
...
def episode(env: Big2Env, policy: MLPPolicy, device='cpu'):
    ...
    while True:
        ...
        # Print player's hand and action
...
def evaluate_against_baseline(current_policy: MLPPolicy, baseline_policy: MLPPolicy, 
                              num_games: int = 100, device='cpu') -> float:
    """
...
def train_selfplay(episodes=2000, lr=3e-4, entropy_beta=0.01, value_coef=0.5, gamma=1.0, seed=42, device='cpu', eval_interval=50, eval_games=100):
    random.seed(seed)
    np.random.seed(seed)
    ...
    for ep in range(1, episodes+1):
        ...
        if ep % eval_interval == 0:
            ...
            if baseline_policy is not None:
                ...
                print(f"[Episode {ep}] Win rate vs baseline: {win_rate:.2%} ({int(win_rate * eval_games)}/{eval_games} wins)")
                print(f"[Episode {ep}] loss={loss.item():.3f} pol={policy_loss.item():.3f} val={value_loss.item():.3f} ent={entropy_term.item():.3f} steps/player~{avg_len:.1f}\n")
            else:
                ...
                baseline_policy.eval()
                print(f"[Episode {ep}] loss={loss.item():.3f} pol={policy_loss.item():.3f} val={value_loss.item():.3f} ent={entropy_term.item():.3f} steps/player~{avg_len:.1f}")
                print(f"[Episode {ep}] Saving baseline checkpoint (no evaluation yet)\n")
...
def plot_training_curves(loss_history, policy_loss_history, value_loss_history, entropy_history, 
                         eval_episodes=None, win_rates=None, save_path='training_curves.png'):
    ...
    if eval_episodes and win_rates and len(axes) > 4:
        ...
        axes[4].set_title('Win Rate vs Previous Checkpoint\n(Current policy as Player 0 vs 3 baseline copies)', 
...
if __name__ == '__main__':
    ...
    policy, loss_history, policy_loss_history, value_loss_history, entropy_history, eval_episodes, win_rates = train_selfplay(
        episodes=1000, lr=0.001, entropy_beta=0.01,
        value_coef=0.5, gamma=0.9, seed=42, device=device,
    ...
    plot_training_curves(loss_history, policy_loss_history, value_loss_history, entropy_history, 
                        eval_episodes, win_rates)