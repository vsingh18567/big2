...
class TestIsConsecutive:
    ...
    def test_consecutive_valid(self):
        # 3, 4, 5, 6, 7
        assert is_consecutive([0, 1, 2, 3, 4])

        # 9, 10, J, Q, K (ranks 6-10)
        assert is_consecutive([6, 7, 8, 9, 10])

    def test_consecutive_with_ace_high(self):
        # 10, J, Q, K, A (ranks 7-11)
        assert is_consecutive([7, 8, 9, 10, 11])

    def test_not_consecutive_with_2(self):
        # 2 (rank 12) cannot be in a straight in Big 2
        assert not is_consecutive([8, 9, 10, 11, 12])
        assert not is_consecutive([12])

    def test_not_consecutive_gap(self):
        # Has a gap
        assert not is_consecutive([0, 1, 3, 4, 5])
        assert not is_consecutive([0, 2, 4, 6, 8])

    def test_not_consecutive_duplicate(self):
        # Duplicates break consecutiveness
        assert not is_consecutive([0, 0, 1, 2, 3])

    def test_single_element(self):
        assert is_consecutive([5])

    def test_two_consecutive(self):
        assert is_consecutive([5, 6])
        assert not is_consecutive([5, 7])
