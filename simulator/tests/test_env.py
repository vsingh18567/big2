...
from simulator.cards import (
    CARDS_PER_DECK, Combo, PASS, SINGLE, PAIR, STRAIGHT, 
    hand_to_combo, 
    card_rank, compare_combos
)
...
class TestEnv:
    ...
    def test_passing_mechanism(self):
        ...
        # Check observation shows no last play
        last_play_portion = obs[13:65]
        assert np.sum(last_play_portion) == 0
    ...
    def test_legal_candidates_fresh_trick(self):
        ...
        has_three_diamonds = any(
            c.cards == [0] for c in candidates
        )
    ...
    def test_legal_candidates_must_follow(self):
        ...
        # Play a single card
        initial_player = env.current_player
        env.step(hand_to_combo([0]))  # Play 3 of diamonds
    ...
    def test_five_card_combo_play(self):
        ...
        # 3 of diamonds=0, 4 of clubs=5, 5 of hearts=10, 6 of spades=15, 7 of diamonds=20
        ...
        assert env.trick_pile.type == STRAIGHT
    ...
    def test_combo_comparison_in_gameplay(self):
        ...
        for c in non_pass:
            assert c.type == SINGLE
            result = compare_combos(c, hand_to_combo([0]))
            assert result > 0
