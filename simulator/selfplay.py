...
def detect_combo(cards: List[int]) -> Optional[Combo]:
...
def generate_straights(hand: List[int]) -> List[List[int]]:
    ...
    straights = []
    ranks_available = sorted(by_rank_suits.keys())
...
class Big2Env:
    ...
    def reset(self):
        ...
        self.trick_pile: Optional[Combo] = None  # None means new trick (no constraint)
    ...
    def step(self, action: Combo) -> Tuple[np.ndarray, int, bool, Dict]:
...
class Big2PolicyValue(nn.Module):
    def __init__(self, card_vocab=53, card_emb_dim=32, state_dim=120+0, hidden=256, action_hidden=128, device='cpu'):
        super().__init__()
        self.device = device
    ...
    def forward(self, state_tensor: torch.Tensor, actions_batch: List[List[Tuple[np.ndarray, Tuple]]]):
...
def select_action(policy: Big2PolicyValue, state: np.ndarray, candidates: List[Combo]) -> Tuple[Combo, torch.Tensor, torch.Tensor, torch.Tensor]:
...
def train_selfplay(episodes=2000, lr=3e-4, entropy_beta=0.01, value_coef=0.5, gamma=1.0, seed=42, device='cpu'):
    random.seed(seed)
    np.random.seed(seed)
...
def marginal_value_of_card(policy: Big2PolicyValue, card_id: int, sims: int=1024, device='cpu') -> float:
    ...
    for s in range(sims):
        ...
        baseline = value_of_starting_hand(policy, sorted(rest + [repl], key=lambda c: (card_rank(c), card_suit(c))), sims=1, device=device)
...
if __name__ == '__main__':
    ...
    policy = train_selfplay(episodes=args.episodes, lr=args.lr, entropy_beta=args.entropy_beta,
                            value_coef=args.value_coef, gamma=args.gamma, seed=args.seed, device=device)
